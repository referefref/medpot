...
func main() {
    ...
    for {
        conn, err := l.Accept()
        if err != nil {
            cconf_t.logger.Error("Failed to accept connection", zap.Error(err))
            notify.Error(fmt.Sprintf("Failed to accept connection: %s", err.Error()), "medpot.main()")
            continue
        }

        go func(conn net.Conn) {
            cconf_t.conn = conn
            handleRequest(cconf_t)
        }(conn)
    }
}
...
func handleRequest(cconf_t *conf_t) {
    defer func() {
        notify.Warning("Closing connection")
        cconf_t.conn.Close()
        cconf_t.logger.Info("Connection closed")
    }()

    buf := make([]byte, 1024*2) // Correct buffer size
    counter := 0

    for counter < 3 {
        timeoutDuration := 5 * time.Second // Increase timeout duration
        cconf_t.conn.SetReadDeadline(time.Now().Add(timeoutDuration))

        reqLen, err := cconf_t.conn.Read(buf)
        if err != nil {
            if netErr, ok := err.(net.Error); ok && netErr.Timeout() {
                cconf_t.logger.Warn("Connection timed out", zap.Error(err))
                notify.Warning(fmt.Sprintf("Connection timed out: %s", err.Error()))
            } else if err == io.EOF {
                cconf_t.logger.Info("Client closed the connection", zap.Error(err))
                notify.Warning(fmt.Sprintf("Client closed the connection: %s", err.Error()))
                return
            } else {
                cconf_t.logger.Error("Error reading from connection", zap.Error(err))
                notify.Error(fmt.Sprintf("Error reading from connection: %s", err.Error()), "medpot.handleRequest()")
                return
            }
            counter++
            ...
            cconf_t.encoded = base64.StdEncoding.EncodeToString(bufTarget)
